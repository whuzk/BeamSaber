    ...
    for utt_ind=1:length(mat),
      ...
      for f = 1:nbin,
        for t = 1:nfram,
          ...
          Df = exp(-2*1i*pi*fs*TDOA(:,t)); % steering vector

          % superdirective beamforming
          mvdr(f,t) = Df(~fail)'/...
                      (Ncov(~fail,~fail,f)+wng*eye(size(diag(Xspec(~fail,f)))))* ...
                      Xtf(~fail)/...
                      (Df(~fail)'/(Ncov(~fail,~fail,f)+wng*eye(size(diag(Xspec(~fail,f)))))*Df(~fail));
          % wng number = abs(mvdr' * Df)^2 / mvdr' * w
          noisegain = abs(mvdr(f,t)' * Df(~fail)).^2 / mvdr(f,t)' * mvdr(f,t);
          noiser = log10(sum(noisegain));
          if real(noiser) ~= -10,
            wng = wng + 0.05;
          end
          % disp(real(noiser));
