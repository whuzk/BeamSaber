function [snr_mean, segsnr_mean]= comp_snr_n(cleanFile, enhdFile)
%
...
%    noise_energy  = sum((clean_frame-processed_frame).^2);
   signal_energy = var(clean_frame);
   noise_energy  = var(clean_frame-processed_frame);
%    segmental_snr(frame_count) = 10*log10(signal_energy/(noise_energy+eps)+eps);
   ...
   if segmental_snr(frame_count)<MIN_SNR
       segmental_snr(frame_count) = MIN_SNR;
   end
