function new_omlsa(fin,fout,ftrans)
% Transient reducing OM-LSA: Single Channel OM-LSA with IMCRA and Transient estimators
% ***************************************************************@
...
N_in=wavread(fin,'size');
N_in = N_in(1);
N_trans=wavread(ftrans,'size');
N_trans=N_trans(1);
N=min(N_in,N_trans);
...
out=zeros(M,1);
lambda_dout = zeros(M, 1);
lambda_davout = zeros(M, 1);
...
for l=1:Nframes
    if l==1
        ...
        [trans,Fs_trans,wmode_trans,fidx_trans]=readwav(ftrans,'rf',M,0);    %our: open transient file and read one frame of data
    else
        [y0,Fs,wmode,fidx]=readwav(fidx,'rf',Mno);
        y=[y(Mno+1:M,:) ; y0];      % update the frame of data
        [trans0,Fs_trans,wmode_trans,fidx_trans]=readwav(fidx_trans,'rf',Mno);%our
        trans=[trans(Mno+1:M,:) ; trans0];      % our:update the frame of transient
    end
    ...
    if (~fnz_flag && abs(y(1))>zero_thres) ||  (fnz_flag && any(abs(y)>zero_thres))       % new version omlsa3
        ...
        Y=fft(win.*y);
        Ya2=abs(Y(1:M21)).^2;
        TRANS=fft(win.*trans);%our
        
         lambda_t=abs(TRANS(1:M21)).^2;%%% transient spectrum
%          Ya2=Ya2-0.003*lambda_t;
        %         if l==1     % new version omlsa3
        if l==l_fnz     % new version omlsa3
            lambda_d=Ya2;
        end
%         gamma=Ya2./max(lambda_d,1e-10);
         gamma=Ya2./max((lambda_d+lambda_t),1e-10);%%% insertion of transient spectrum
...
%          gamma=Ya2./max(lambda_d,1e-10);
         gamma=Ya2./max((lambda_d+lambda_t),1e-10);%our: insertion of transient spectrum
    ...
    if l==1
        fwidx=writewav(out(1:Mno),Fs,fout,'rf',0);  % open output file and write first output samples
    else
        fwidx=writewav(out(1:Mno),Fs,fwidx,'rf');  % write to output file
    end
    out=[out(Mno+1:M); zeros(Mno,1)];   % update output frame
    %lambda_dout =[lambda_d(Mno+1:M); zeros(Mno,1)];
    %lambda_davout =[lambda_davout(Mno+1:M); zeros(Mno,1)];
    if ~mod(l,Ncount)
...
lambda_t_mat=[lambda_t_mat,lambda_t];

end
close(waitHandle)
fwidx=writewav(out(1:M-Mno),Fs,fwidx,'rf',0);  % write to output file the last output samples
%audiowrite('lambda_d.wav', lambda_d, Fs);  % write to output file the last output samples
%writewav('lambda_dav',lambda_dav,Fs);  % write to output file the last output samples
fclose(fidx(1));    % close input file
fclose(fwidx(1));   % close output file
