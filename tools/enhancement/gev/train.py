...
python3 train.py --chime_dir=../../../../../CHiME4/CHiME3/data/ ../../../../../CHiME4/CHiME3/data/audio/16kHz/gev_babble7min --initmodel=data/BLSTM_model/best.nnet --gpu 0 BLSTM
...
if args.chime_dir != '':
    log.info(
            'Preparing training data and storing it in {}'.format(
                    args.data_dir))
    prepare_noise_training_data(args.chime_dir, args.data_dir)
...
while (epoch < args.max_epochs and not exhausted):
    ...
    if loss_cv < best_cv_loss:
        ...
        model_file = os.path.join(model_save_dir, 'chime_clean_9x_noise_transfer[31042017].nnet')
        log.info('New best loss during cross-validation. Saving model file '
                 'under {}'.format(model_file))
        serializers.save_hdf5(model_file, model)
        serializers.save_hdf5(os.path.join(model_save_dir, 'chime_clean_9x_noise_transfer[31042017].tr'), optimizer)
